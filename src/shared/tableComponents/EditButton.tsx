...
import {removeEstintore, setEstintoreSelezionato} from "../../store/estintoreSlice";
...
const EditButton: React.FC<EditButtonProps> = (
    {
        estintoreTarget, setEditabile, setModifica
    }
) => {
    ...
    return (
        <>
            <div className="text-right">
                <label htmlFor="my-modal-4" className="mr-4"
                       onClick={() => {
                ...
                <div className="tooltip tooltip-left tooltip-info" data-tip="Modifica">
                    <label htmlFor="my-modal-4" className="btn btn-link btn-xs hover:bg-sky-500"
                           onClick={() => {
                               dispatch(setEstintoreSelezionato(estintoreTarget))
                               setEditabile(true)
                               setModifica(true)
                ...
                <div className="tooltip tooltip-left tooltip-info z-10" data-tip="Elimina">
                    <button className="btn btn-link btn-xs hover:bg-sky-500"
                            onClick={() => {
                                execQuery(deleteEstintoreFromFauna, estintoreTarget?.faunaDocumentId).then(() => {
                                    dispatch(removeEstintore(estintoreTarget?.faunaDocumentId as string))
                                })
                            }}
                    >
                        <svg
