...
import {
  ...
  resetItem,
  setRicerca,
  TotalControlItemsSelector,
  TotalControlRicercaByGruSelector,
  TotalControlRicercaByImpresaSelector,
  TotalControlRicercaByMacchinaEAttrezzaturaSelector,
  TotalControlRicercaByMaestranzaSelector,
  TotalControlRicercaByPonteggioSelector,
...
import { PonteggioSelector, setPonteggioSelezionato } from "../../store/ponteggioSlice";
import CreazioneGru from "../mirkoComponents/modal/CreazioneGru";
import { GruSelector, setGruSelezionata } from "../../store/gruSlice";
import { MaestranzeSelector, setMaestranzaSelezionata } from "../../store/maestranzaSlice";
...
const TotalControl: React.FC<TotalControlProps> = ({}) => {
  ...
  const ricercaByMaestranza = useSelector(
    TotalControlRicercaByMaestranzaSelector
  );
  const ricercaByMacchinaEAttrezzatura = useSelector(
    TotalControlRicercaByMacchinaEAttrezzaturaSelector
  );
  const ricercaByPonteggio = useSelector(
    TotalControlRicercaByPonteggioSelector
  );
  ...
  const [modificaEffettuata, setmodificaEffettuata] = useState(false)

  const [itemsFiltered, setItemFiltered] = useState<
    {
      item: Impresa | MacchinaEAttrezzatura | Maestranza | Ponteggio | Gru;
      tipo:
        | "Impresa"
        | "Maestranza"
    ...
    imprese.forEach((i) => {
      execQuery2(getAllMacchineEAttrezzatureByCreatoDa, i.id).then((res) => {
        res.Items.forEach((item: any) => {
          ...
          if (
            r &&
            Date.parse(r.ultimaRevisione.scadenza) - Date.now() <
              45 * 24 * 3600 * 1000
          ) {
            dispatch(
              addItem({
                ...
                scadenza: r.ultimaRevisione.scadenza,
                problema: "Ultima Revisione",
              })
            );
          }
        });
        ...
        res.Items.forEach((item: any) => {
          ...
          if (maestranza) {
            maestranza.documenti.forEach((d) => {
              if (
                Date.parse(d.scadenza as string) - Date.now() <
                45 * 24 * 3600 * 1000
              ) {
                dispatch(
                  addItem({
  ...
  useEffect(() => {
    if (ricercaByImpresa && ricercaByImpresa !== "") {
      let impresa = imprese.filter(i => {
        return i.anagrafica.attr
              .filter((a) => a.label === "denominazione")[0]
              .value.toLowerCase()
              .replace(/\s+/g, "")
      ...
      let itemsRicerca = items.filter((i) => {
        if (i.tipo !== "Impresa") {
          if (
            (
              i.item as Maestranza | MacchinaEAttrezzatura | Ponteggio | Gru
            ).creatoDa
              .replace(/\s+/g, "")
              .includes(impresa.length > 0 ? impresa[0].id as string : "")
          ) {
            return i;
          }
        }
      ...
      let itemsRicerca = items.filter((i) => {
        if (i.tipo === "Maestranza") {
          if (
            (
              (i.item as Maestranza).anagrafica.filter(
                (m) => m.label === "nome"
              )[0].value as string
            )
              .toLowerCase()
              .replace(/\s+/g, "")
              .includes(
                ricercaByMaestranza.toLowerCase().replace(/\s+/g, "")
              ) ||
            (
              (i.item as Maestranza).anagrafica.filter(
                (m) => m.label === "cognome"
      ...
      ricercaByMacchinaEAttrezzatura &&
      ricercaByMacchinaEAttrezzatura !== ""
    ) {
      let itemsRicerca = items.filter((i) => {
        if (i.tipo === "MacchinaEAttrezzatura") {
          if (
            (i.item as MacchinaEAttrezzatura).attr
              .filter((a) => a.nome === "denominazione")[0]
              .value.toLowerCase()
              .replace(/\s+/g, "")
              .includes(
                ricercaByMacchinaEAttrezzatura.toLowerCase().replace(/\s+/g, "")
              )
          ) {
            return i;
          }
      ...
      let itemsRicerca = items.filter((i) => {
        if (i.tipo === "Ponteggio") {
          if (
            (
              (i.item as Ponteggio).attr.filter(
                (a) => a.nome === "tipologia"
              )[0].value as string
            )
              .toLowerCase()
      ...
      let itemsRicerca = items.filter((i) => {
        if (i.tipo === "Gru") {
          if (
            (
              (i.item as Gru).attr.filter((a) => a.nome === "tipologia")[0]
                .value as string
            )
              .toLowerCase()
              .replace(/\s+/g, "")
    ...
    <>
      <div className="flex flex-col items-center w-100">
        ...
        <div className="grid grid-cols-5 gap-10 mt-20">
          ...
          />
          <SearchComponent
            placeholder="Maestranza"
            onChangeFunction={(ev) => {
              dispatch(
                setRicerca({ testo: ev.target.value, tipo: "Maestranza" })
              );
            }}
          />
          <SearchComponent
            placeholder="Macchina/Attrezzatura"
            onChangeFunction={(ev) => {
              dispatch(
                setRicerca({
                  testo: ev.target.value,
                  tipo: "MacchinaEAttrezzatura",
                })
              );
            }}
          />
          <SearchComponent
            placeholder="Ponteggio"
            onChangeFunction={(ev) => {
              dispatch(
                setRicerca({ testo: ev.target.value, tipo: "Ponteggio" })
              );
            }}
          />
          <SearchComponent
            ...
            <tbody>
              ...
              {itemsFiltered.map((i, index) => {
                if (i.tipo === "MacchinaEAttrezzatura") {
                  return (
                    <tr
                      key={(i.item as MacchinaEAttrezzatura).id}
                      className="link link-hover hover:text-sky-500"
                    >
                      <th>{index + 1}</th>
                      <td>{imprese.filter(im => im.id === i.item.creatoDa)[0].anagrafica.attr.filter((a) => a.label === "denominazione")[0].value}</td>
                      <td>
                        {
                          (i.item as MacchinaEAttrezzatura).attr.filter(
                            (a) => a.nome === "denominazione"
                          )[0].value
                        }
                      </td>
                      <td>{i.problema}</td>
                      <td>Macchine e Attrezzature</td>
                      <td>
                        {Date.parse(i.scadenza) - Date.now() < 0 ? (
                          <div
                            className="tooltip tooltip-left"
                            data-tip="Scaduto"
                          >
                            <MdOutlineBlock color="red" size={30} />
                          </div>
                        ) : (
                          <div
                            className="tooltip tooltip-left"
                            data-tip={`${
                                Date.parse(i.scadenza) - Date.now() <
                                30 * 24 * 3600 * 1000
                                  ? "Scadenza Imminente"
                                  : "In Scadenza"
                              }`}
                      ...
                      <td className="hover:cursor-pointer">
                        <label
                          className="hover:cursor-pointer hover:underline hover:text-black"
                          htmlFor="my-modal-7"
                          onClick={() => {
                            dispatch(
                              setImpresaSelezionata(
                                imprese.filter(
                                  (im) =>
                                    im.id ===
                                    (i.item as MacchinaEAttrezzatura).creatoDa
                                )[0]
                              )
                            );
                            dispatch(
                              setMacchinaEAttrezzaturaSelezionato(
                                i.item as MacchinaEAttrezzatura
                              )
                            );
                            setModifica(true)
                          }}
                          ...
                          onClick={() => {
                            dispatch(
                              setImpresaSelezionata(
                                imprese.filter(
                                  (im) =>
                                    im.id === (i.item as Maestranza).creatoDa
                                )[0]
                              )
                            );
                            dispatch(
                              setMaestranzaSelezionata(i.item as Maestranza)
                            );
                            setModifica(true)
                          }}
                        >
                          ...
                          onClick={() => {
                            dispatch(
                              setImpresaSelezionata(
                                imprese.filter(
                                  (im) =>
                                    im.id === (i.item as Ponteggio).creatoDa
                                )[0]
                              )
                            );
                            dispatch(
                              setPonteggioSelezionato(i.item as Ponteggio)
                            );
                            setModifica(true)
                          }}
                        >
                        ...
                        <label
                          ...
                          onClick={() => {
                            dispatch(
                              setImpresaSelezionata(
                                imprese.filter(
                                  (im) => im.id === (i.item as Gru).creatoDa
                                )[0]
                              )
                            );
      ...
      <CreazioneMacchinaEAttrezzatura
        editabile={true}
        modifica={true}
      ...
      <CreazioneGru
        ...
        setmodificaEffettuata={setmodificaEffettuata}
      />
      <CreazioneMaestranzaModale editabile={true} modifica={true} setmodificaEffettuata={setmodificaEffettuata}/>
    </>
  );
};
