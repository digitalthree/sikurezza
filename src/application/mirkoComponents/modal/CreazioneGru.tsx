...
const CreazioneGru: React.FC<CreazioneGruProps> = ({
  ...
  const onSubmit = async (gru: Gru) => {
    setSave(true);
    setsaving(true);
    // 2. Prepara un array per contenere tutte le Promise di upload S3
    const uploadPromises = gru.documenti
    ...
    <>
      ...
      <label htmlFor="my-modal-5" className="modal cursor-pointer">
        <label className="modal-box relative max-w-7xl">
          ...
          {gru.documenti.map((d) => {
            return (
              <div className="grid grid-cols-7 mb-3">
                ...
                {typeof gru.documenti.filter((doc) => doc.nome === d.nome)[0]
                  .presenza === "boolean" ? (
                  <div className="col-span-1 flex flex-row">
                    NO
                    <input
                      type="checkbox"
                      className="toggle col-span-1 mr-2 ml-2"
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                        }
          ...
          {editabile && (
            <div className="modal-action" onClick={() => onSubmit(gru)}>
              <label htmlFor="my-modal-5" className="btn btn-warning w-full">
                {editabile && !modifica ? "Crea Gru" : "Modifica Gru"}
              </label>
            </div>
          )}
        </label>
