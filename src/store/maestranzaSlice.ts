...
export const MaestranzaSlice = createSlice({
    ...
    reducers: {
        ...
        setDataAssunzioneIlInMaestranza(state: MaestranzaState, action: PayloadAction<string>){
            state.maestranzaDaCreare.documenti.forEach((d) => {
                if(d.nome === 'contratto'){
                    d.dataAssunzione = action.payload
                }
            })
            if(state.maestranzaSelezionata){
                state.maestranzaSelezionata.documenti.forEach((d) => {
...
export const {
    addMaestranzaToMaestranzaSlice, removeMaestranzaToMaestranzaSlice, setMaestranzaSelezionata, setAnagraficaMaestranza,
